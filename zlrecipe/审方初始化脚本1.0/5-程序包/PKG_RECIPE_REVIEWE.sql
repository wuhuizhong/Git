CREATE OR REPLACE PACKAGE PKG_RECIPE_REVIEWE IS
  ...
  --药师科室分配设置
  Procedure Upd_pharmacist_reviewe_dept(dept_id in clob,user_id in varchar2,V_Current_USER_CODE in varchar2,N_NUMBER in number);
...
--取消启用科室
 Procedure DEL_REVIEWE_DEPT( DEPT_NAME_IN in varchar2);
  ...
  --参数设置
  PROCEDURE PARAMETER_SETTING( ZLKBC_SERVER_IN in varchar2,
...
(50) PATH '//info[@name="肝功能不全"]/@value',严重肝功能不全 VARCHAR2(50) PATH '//info[@name="严重肝功能不全"]/@value',肾功能不全 VARCHAR2(50) PATH '//info[@name="肾功能不全"]/@value',严重肾功能不全 VARCHAR2(50) PATH '//info[@name="严重肾功能不全"]/@value',主要诊断 VARCHAR2(1000) PATH '//info[@name="诊断名称"]/@value'
  ...
  EXCEPTION
    WHEN OTHERS THEN
      PKG_RECIPE_REVIEWE.INSERT_LOG(
        LOG_TITILE_IN    => '审方分配'
        ,LOG_TYPE_IN      => 'E'
        ,LOG_CONTENT_IN   => USER_ID || ',' || SQLERRM
      );
...
-- 药师科室分配设置
    ...
    --功能说明：-- 药师对应科室分配设置
    --入参说明： -- user_id 为界面上获取的 当前光标指向的用户ID，dept_name 科室名称串  内一科，内二科，... N_NUMBER - 1 删除分配科室， 2 新增分配科室 
    --出参说明：
    --编 写 者：吴开波
    --编写时间：2018-06-27
    --版本记录：版本号+时间+修改者+修改需求描述
  ...
  from PHARMACIST_REVIEWE_DEPT t
 where t.user_code = USER_ID
   and INSTR(V_DEPT_ID,',' || t.dept_id || ',') > 0
  ) LOOP
 ...
 --只新增启用科室
    FOR I IN ( SELECT A.DEPT_ID,A.DEPT_NAME,A.LAST_EDITOR
                 FROM REVIEWE_DEPT A
                WHERE INSTR(
      V_DEPT_ID
      ,',' || A.DEPT_ID || ','
...
--科室取消启用
 ...
 --DEPT_STATUS number,
  ...
  --查询病人处方审方结果
    ...
    --查询当前在岗药师,获取药师code 串
    SELECT COUNT(*) INTO N_COUNT FROM PHARMACIST_WORK_RECORD WHERE register_date>=V_审核日期 AND unregister_time is null ;
    ...
    IF N_COUNT=0 THEN
      ...
      --当前有药师在岗
      select 
      listagg(t.pharmacist_id,',') Within Group(order by t.pharmacist_id) into V_PHARMACIST_ID
    from PHARMACIST_WORK_RECORD t where t.register_date>=V_审核日期 and t.unregister_time is null ;
    end if ;
    
		OPEN RESULTOUT FOR SELECT DISTINCT ORDER_ID
		                                  ,ORDER_GROUP_ID
                                      ,nvl(PHARMACIST_ID,V_PHARMACIST_ID) PHARMACIST_ID
		                                  ,CASE
...
---审方方案设计
  ...
  --修改审核方案明细
    ...
    --版本记录：版本号+时间+修改者+修改需求描述
    --头注释结束-----------------------------------------------------------------------------------------
		V_REVIEWE_SOLUTION_ID   VARCHAR2(2000);
	BEGIN
		IF DBMS_LOB.GETLENGTH(INPUT_IN) <> 0 THEN
			FOR I IN ( SELECT REVIEWE_SOLUTION_ID
			                 ,LAST_EDITOR
...
Begin 
  ...
  select max(para_value) into v_审方定时刷新 from recipe_reviewe_para where PARA_NAME = '审方定时刷新间隔';  
  -- 修改 合理用药服务器
 ...
 --修改 审方定时刷新间隔时间 单位秒
  if TIMER_REFRESH <> v_审方定时刷新 then
  UPDATE recipe_reviewe_para
     SET para_value     = TIMER_REFRESH,
         LAST_EDITOR    = V_USER_NAME,
         LAST_EDIT_ITME = v_当前时间
   where PARA_NAME = '审方定时刷新间隔';
 end if;
Exception
  When Others Then
    PKG_RECIPE_REVIEWE.INSERT_LOG(
