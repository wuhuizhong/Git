CREATE OR REPLACE PACKAGE PKG_RECIPE_COMMENT AS
  ...
  --完成点评（具体医嘱或处方）
	...
	PROCEDURE AUTO_COMMENT_RECIPE (
		EXTRACT_ID_IN IN NUMBER,err_return out varchar2
	);
 --人工点评
 ...
 --抽取点评处方
  ...
  --头注释结束----------------------------------------------------------------------------------------- 
	BEGIN
		...
		END IF;
		IF EXTRACT_ID_OUT IS NOT NULL THEN
			V_RESULT_STATUS   := 'OK';
		ELSE
		...
		END IF;
	EXCEPTION
		WHEN OTHERS THEN --如果有错误，抽取ID返回空给页面
			V_RESULT_STATUS   := SUBSTR(
				SQLERRM
				,1
				,4000
			);
...
--头注释结束-----------------------------------------------------------------------------------------
  ...
  --抽取处方
		INSERT   INTO COMMENT_RECIPES (
			...
			SELECT EXTRACT_ID_OUT
			      ...
			      ,RECIPE_DR
			      ,RECIPE_DR_TITLE
			      ,MAX(DR_ANTI_LEVEL) DR_ANTI_LEVEL
			      ,MAX(100) HIS_NO
			      ,COUNT(U.ID) RECIPE_DRUG_NUMBER
			      ,MAX(ANTI_DRUG_MAX_LEVEL) ANTI_DRUG_MAX_LEVEL
			      ,MAX(BASE_DRUG_NUMBER) BASE_DRUG_NUMBER
			      ,MAX(PATIENT_AGE) PATIENT_AGE
			      ,MAX(PATIENT_BIRTHDAY) PATIENT_BIRTHDAY
			      ,MAX(RECIPE_DATE) RECIPE_DATE
			      ,SUM(U.实收金额) RECIPE_AMOUNT
			      , ( SELECT
				LISTAGG(诊断描述
				,'|') WITHIN  GROUP(
					 ORDER BY ''
			   ...
			   AND LL.相关ID = JJ.医嘱ID
			   AND LL.ID = KK.医嘱ID
			   AND KK.NO = P.RECIPE_NO
			)
			) RECIPE_DIAG
			      ,RECIPE_TYPE
			  FROM ( SELECT EXTRACT_ID_OUT
			               ,RECIPE_NO
			               ,PID
			               ,PVID
			               ,RECIPE_DR
			           ...
			           FROM ( SELECT RECIPE_NO
			                        ...
			                        ,MAX(PATIENT_BIRTHDAY) PATIENT_BIRTHDAY
			                        ,MAX(RECIPE_DATE) RECIPE_DATE
			                    FROM ( SELECT distinct C.NO RECIPE_NO
			                                 ,C.记录性质
			                                 ,A.病人ID PID
			                                 ,A.挂号单 PVID
			                                 ,A.开嘱医生 RECIPE_DR
			...
			,记录性质
			 ORDER BY DBMS_RANDOM.VALUE ) E
			          WHERE RECIPE_TYPE <> 1 and ROWNUM <= RECIPES_NUMBER_IN
			) P
			      ,门诊费用记录@TO_ZLHIS U
			 WHERE U.记录性质 = P.记录性质
			   AND U.记录状态 IN (
			   ...
			   AND U.NO = P.RECIPE_NO
			 GROUP BY P.RECIPE_DR_TITLE
			,RECIPE_TYPE
			,EXTRACT_ID_OUT
			,RECIPE_NO
			,PID
  ...
  --完成点评（具体医嘱或处方）
    ...
    --头注释结束-----------------------------------------------------------------------------------------
		N_PRO_COUNT   NUMBER;
	BEGIN
		IF RECIPE_NO_IN IS NOT NULL THEN
			SELECT COUNT(*)
			  INTO N_PRO_COUNT
			  FROM COMMENT_RESULT
