...
CREATE OR REPLACE Package Body Pkg_Status_Record Is
  ...
  (
    ...
    --头注释结束-----------------------------------------------------------------------------------------
    Pat_Xml      Sys.Xmltype;
    Pat_Info_Xml Sys.Xmltype;
    c_Clob       Clob;
  Begin
    Pat_Xml := Xmltype('<IN><PATIID>4900</PATIID><PAGEID>1</PAGEID></IN>');

    --zlhis.Zl_Third_Getpatiinfo_Ydhl@to_zlhis(Xml_In=>pat_xml,Xml_Out=>pat_info_xml);
    c_Clob := Pat_Info_Xml.Getclobval();
    Dbms_Output.Put(c_Clob);
  End Get_Pat_Info;
End;
